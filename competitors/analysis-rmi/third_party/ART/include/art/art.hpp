...
class ART {
    ...
    private:
    Node* tree_ = NULL;
    const std::vector<KeyValue<uint64_t, std::size_t>> *data_;
    std::size_t n_keys_;
    std::size_t sparcity_;
    static uint64_t allocated_byte_count_ ;

    public:
    ART(const std::vector<KeyValue<uint64_t, std::size_t>> &data, const std::size_t sparcity = 1) {
        data_ = &data;
        n_keys_ = data.size();
        sparcity_ = sparcity;
        allocated_byte_count_ = 0;
    ...
    std::pair<std::size_t, std::size_t> search(const uint64_t lookup_key) {
        uint8_t key[8];
        ...
        auto res = not found ? n_keys_ - 1 : it.value;
        auto lo = res < sparcity_ - 1 ? 0 : res - (sparcity_ - 1);
        auto hi = std::min<std::size_t>(n_keys_, res + 1);

        return std::make_pair(lo, hi);
    }

    std::size_t size_in_bytes() const {
        return sizeof(*this) + allocated_byte_count_;
    }
    ...
    */
    ...
    struct Node {
        ...
        ~Node() {
            switch (type) {
                case NodeType4: {
                    allocated_byte_count_ -= sizeof(Node4);
                    break;
                }
                case NodeType16: {
                     allocated_byte_count_ -= sizeof(Node16);
                     break;
                }
                case NodeType48: {
                     allocated_byte_count_ -= sizeof(Node48);
                     break;
                 }
                case NodeType256: {
                    allocated_byte_count_ -= sizeof(Node256);
                    break;
                }
            }
        }
    ...
    struct Node4 : Node {
        ...
        Node4() : Node(NodeType4) {
            ...
            allocated_byte_count_ += sizeof(*this);
        }
    };
...
uint64_t ART::allocated_byte_count_;
