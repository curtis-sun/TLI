...
// the data array.
namespace sosd_art_int{

template<class KeyType>
class ART_INT : public Base<KeyType> {
 public:
  ART_INT(const std::vector<int>& params){}
  ~ART_INT() { destructTree(tree_); }
  ...
  uint64_t Build(const std::vector<KeyValue<KeyType>>& data, size_t num_threads) {
    ...
    uint64_t timing = util::timing([&] {
      ...
      for (size_t i = 0; i < data.size(); ++i) {
        data_[i].value = data[i].value;
        data_[i].key = std::move(util::convertToString(data[i].key));
      }
 ...
 private:
  ...
  typedef val_type std::pair<uint8_t[sizeof(KeyType)], uint64_t>;
 ...
 */
  ...
  static const int8_t LeafNodeType4 = 4;
  static const int8_t LeafNodeType16 = 5;
  static const int8_t LeafNodeType48 = 6;
  static const int8_t LeafNodeType256 = 7;
  ...
  struct Node {
    ...
    ~Node() {
      switch (type) {
        ...
        case LeafNodeType4: {
          allocated_byte_count -= sizeof(LeafNode4);
          break;
        }
        case LeafNodeType16: {
          allocated_byte_count -= sizeof(LeafNode16);
          break;
        }
        case LeafNodeType48: {
          allocated_byte_count -= sizeof(LeafNode48);
          break;
        }
        case LeafNodeType256: {
          allocated_byte_count -= sizeof(LeafNode256);
          break;
        }
      }
    }
  ...
  struct LeafNode16 : Node {
    uint8_t key[16];
    val_type kv[16];

    Node16() : Node(LeafNodeType16) {
      memset(key, 0, sizeof(key));
      memset(kv, 0, sizeof(kv));
      allocated_byte_count += sizeof(*this);
    }
  ...
  struct LeafNode48 : Node {
    uint8_t childIndex[256];
    val_type kv[48];

    Node48() : Node(LeafNodeType48) {
      memset(childIndex, emptyMarker, sizeof(childIndex));
      memset(kv, 0, sizeof(kv));
      allocated_byte_count += sizeof(*this);
    }
  ...
  struct LeafNode256 : Node {
    val_type kv[256];

    Node256() : Node(LeafNodeType256) {
      memset(kv, 255, sizeof(kv));
      allocated_byte_count += sizeof(*this);
    }
  };
  ...
  val_type* findVal(Node* n, uint8_t keyByte) const {
    // This address is used to communicate that search failed
    static val_type* nullNode = NULL;
    // Find the next child for the keyByte
    switch (n->type) {
      case LeafNodeType4: {
        LeafNode4* node = static_cast<LeafNode4*>(n);
        for (unsigned i = 0; i < node->count; i++)
          if (node->key[i] == keyByte) return &node->kv[i];
        return &nullNode;
      }
      case LeafNodeType16: {
        LeafNode16* node = static_cast<LeafNode16*>(n);
        __m128i cmp = _mm_cmpeq_epi8(
            _mm_set1_epi8(keyByte),
            _mm_loadu_si128(reinterpret_cast<__m128i*>(node->key)));
        unsigned bitfield = _mm_movemask_epi8(cmp) & ((1 << node->count) - 1);
        if (bitfield)
          return &node->kv[ctz(bitfield)];
        else
          return &nullNode;
      }
      case LeafNodeType48: {
        LeafNode48* node = static_cast<LeafNode48*>(n);
        if (node->childIndex[keyByte] != emptyMarker)
          return &node->kv[node->childIndex[keyByte]];
        else
          return &nullNode;
      }
      case LeafNodeType256: {
        LeafNode256* node = static_cast<LeafNode256*>(n);
        return &(node->kv[keyByte]);
      }
    }
    throw;  // Unreachable
  }
  ...
  bool iteratorNext(Iterator& iter) const {
    ...
    while (!iter.stack.empty()) {
      ...
      switch (node->type) {
        ...
        case LeafNodeType4: {
          LeafNode4* n = static_cast<LeafNode4*>(node);
          if (entry.pos < node->count)
            iter.value = n->kv[entry.pos++];
          return true;
        }
        case LeafNodeType16: {
          LeafNode16* n = static_cast<LeafNode16*>(node);
          if (entry.pos < node->count)
            iter.value = n->kv[entry.pos++];
          return true;
        }
        case LeafNodeType48: {
          LeafNode48* n = static_cast<LeafNode48*>(node);
          for (; entry.pos < 256; entry.pos++)
            if (n->childIndex[entry.pos] != emptyMarker) {
              iter.value = n->kv[n->childIndex[entry.pos++]];
              break;
            }
          return true;
        }
        case LeafNodeType256: {
          LeafNode256* n = static_cast<LeafNode256*>(node);
          for (; entry.pos < 256; entry.pos++)
            if (n->kv[entry.pos].second != util::NOT_FOUND) {
              iter.value = n->kv[entry.pos++];
              break;
            }
          return true;
  ...
  void* minimum(Node* node) const {
    ...
    switch (node->type) {
      ...
      case LeafNodeType48: {
        ...
        unsigned pos = 0;
        while (n->childIndex[pos] == emptyMarker) pos++;
        return &(n->kv[n->childIndex[pos]]);
      }
      case LeafNodeType256: {
        LeafNode256* n = static_cast<LeafNode256*>(node);
        unsigned pos = 0;
        while (n->kv[pos].second == util::NOT_FOUND) pos++;
        return &(n->kv[pos]);
      }
    }
    throw;  // Unreachable
  ...
  bool leafMatches(val_type* leaf, const uint8_t key[], unsigned keyLength,
                   unsigned depth) const {
    // Check if the key of the leaf is equal to the searched key
    const std::string& leafKey = leaf->first;
  ...
  bool leafPrefixMatches(val_type* leaf, const uint8_t key[], unsigned keyLength,
                   unsigned depth) const {
    // Check if the key of the leaf is equal to the searched key
    const std::string& leafKey = leaf->first;
    if (leafKey.length() < keyLength || memcmp(leafKey.c_str() + depth, key + depth, keyLength - depth)){
  ...
  // Compare the the key with the prefix, return the number matching bytes
  {
    ...
    if (compBytes > maxPrefixLength) {
      ...
      val_type* minNode = static_cast<val_type*>(minimum(node));
      const std::string& minKey = minNode->first;
  ...
  inline bool isLeaf(Node* node) const {
    // Is the node a leaf?
    return node->type == LeafNode4 || node->type == LeafNode16 || node->type == LeafNode48 || node->type == LeafNode256;
  }

  bool leafBound(const uint8_t key[], unsigned keyLength, Iterator& iterator, unsigned depth, const std::string& leafKey, bool lower = true){
    unsigned i;
  ...
  bool bound(Node* n, const uint8_t key[], unsigned keyLength, Iterator& iterator,
    ...
    while (true) {
      ...
      if (n->prefixLength) {
        ...
        if (mismatchPos != n->prefixLength) {
          ...
          if (mismatchPos < maxPrefixLength){
            ...
            const std::string& minKey = static_cast<val_type*>(minimum(n))->first;
      ...
      switch (n->type) {
        ...
        case LeafNodeType4: {
          LeafNode4* Leafnode = static_cast<Node4*>(n);
          for (entry.pos = 0; entry.pos < node->count; entry.pos++){
            if (node->key[entry.pos] == keyByte) {
              iterator.stack.push(entry);
              iterator.value = node->kv[entry.pos++];
              leafBound(key, keyLength, iterator, depth, iterator.value.first, lower);
              break;
            } else if (node->key[entry.pos] > keyByte)
              break;
          }
