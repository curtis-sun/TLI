...
public:
    ...
    typedef mabtree<key_type, data_type, SearchClass, value_type, key_compare,
...
public:
    ...
    static const unsigned short predict_threshold = traits::page_size / sizeof(key_type);

    static const unsigned short correct_threshold = predict_threshold; 
...
private:
    ...
    struct node
    {
        ...
        inline bool ispredict() const
        {
            return (node::slotuse > predict_threshold);
        }

        /// True if node has too few entries
        inline bool iscorrect() const
        {
            return (node::midp_err > correct_threshold);
        }
    };
...
public:
    ...
    public:
        ...
        typedef typename mabtree::key_type                key_type;

        /// The data type of the btree. Returned by data().
        typedef typename mabtree::data_type               data_type;

        /// The value type of the btree. Returned by operator*().
        typedef typename mabtree::value_type              value_type;

        /// The pair type of the btree.
        typedef typename mabtree::pair_type               pair_type;
    ...
    private:
        ...
        typename mabtree::leaf_node*      currnode;
        ...
        friend class mabtree<key_type, data_type, SearchClass, value_type, key_compare,
    ...
    public:
        ...
        inline iterator(typename mabtree::leaf_node *l, unsigned short s)
    ...
    private:
        ...
        /// The currently referenced leaf node of the tree
        const typename mabtree::leaf_node*        currnode;
    ...
    public:
        ...
        inline const_iterator(const typename mabtree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
    ...
    public:
        ...
        inline reverse_iterator(typename mabtree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
    ...
    public:
        ...
        inline const_reverse_iterator(const typename mabtree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
...
public:
    ...
    /// comparison function
    explicit inline mabtree(const allocator_type &alloc = allocator_type())
    ...
    /// comparison object
    explicit inline mabtree(const key_compare &kcf,
    ...
    inline mabtree(InputIterator first, InputIterator last,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_allocator(alloc)
    {
        insert(first, last);
    ...
    template <class InputIterator>
    inline mabtree(InputIterator first, InputIterator last, const key_compare &kcf,
                 const allocator_type &alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_key_less(kcf), m_allocator(alloc)
    {
...
public:
    ...
    protected:
        ...
        /// Constructor called from mabtree::value_comp()
...
private:
    ...
    inline int find_lower(const node_type *n, const key_type& key, const unsigned short pos) const
    {
        return SearchClass::lower_bound(n->slotkey, n->slotkey + n->slotuse, key, n->slotkey + pos) - n->slotkey;
    }

    template <typename node_type>
    inline unsigned short predict_pos(const node_type *n, const key_type& key, 
                                      key_type const* lo_key, key_type const* hi_key) const
    {
        if (n->ispredict()){
            BTREE_ASSERT(n->slotuse >= 2);
            int pos = 0;
            if (lo_key && hi_key){
                pos = _predict_correct_pos(n, key, *lo_key, *hi_key, -1);
            }
            else{
                pos = _predict_correct_pos(n, key, n->slotkey[0], n->slotkey[n->slotuse - 1], 0);
            }
            return _strip_pos(n, pos);
        }
        return 0;
    }

    inline unsigned short _strip_pos(const inner_node *n, int pos) const
    {
        pos = (pos < 0) ? 0 : ((pos > n->slotuse) ? n->slotuse : pos);
        return pos;
    }

    inline unsigned short _strip_pos(const leaf_node *n, int pos) const
    {
        pos = (pos < 0) ? 0 : ((pos > n->slotuse - 1) ? n->slotuse - 1 : pos);
        return pos;
    }

    inline int compute_predict_pos(const key_type& key, const key_type& lo_key, const key_type& hi_key, int lo_pos, int hi_pos) const {
        return std::ceil(double(key - lo_key) / (hi_key - lo_key) * (hi_pos - lo_pos)) + lo_pos;
    }

    template<typename node_type>
    int _predict_correct_pos(const node_type *n, const key_type& key, 
                                      const key_type& lo_key, const key_type& hi_key, int lo_pos) const
    {
        int pos = compute_predict_pos(key, lo_key, hi_key, lo_pos, n->slotuse - 1);
        if (n->iscorrect()){
            const int mid = (n->slotuse - 1) / 2;
            const int midp_est = mid + n->midp_err;
            if (pos < midp_est){
                pos -= int(double(pos - lo_pos) / (midp_est - lo_pos) * n->midp_err);
            }
            else if (pos > midp_est){
                pos -= int(double(n->slotuse - 1 - pos) / (n->slotuse - 1 - midp_est) * n->midp_err);
            }
            else{
                pos = mid;
            }
    ...
    template <typename node_type>
    void compute_midp_err(node_type *n){
        if (n->slotuse < 2){
            n->midp_err = 0;
            return;
        }
        const int mid = (n->slotuse - 1) / 2;
        int pos = compute_predict_pos(n->slotkey[mid], n->slotkey[0], n->slotkey[n->slotuse - 1], 0, n->slotuse - 1);
        n->midp_err = pos - mid;
    }
    ...
    template <typename node_type>
    inline int find_upper(const node_type *n, const key_type& key, const unsigned short pos) const
    {
        return SearchClass::upper_bound(n->slotkey, n->slotkey + n->slotuse, key, n->slotkey + pos) - n->slotkey;
    }
...
public:
    ...
    bool exists(const key_type &key) const
    {
        ...
        key_type const* lo_key = NULL;
        key_type const* hi_key = NULL;
        while(!n->isleafnode())
        {
            const inner_node *inner = static_cast<const inner_node*>(n);
            int slot = find_lower(inner, key, predict_pos(inner, key, lo_key, hi_key));
            lo_key = (slot > 0) ? &inner->slotkey[slot - 1] : NULL;
            hi_key = (slot < inner->slotuse) ? &inner->slotkey[slot] : NULL;
        ...
        int slot = find_lower(leaf, key, predict_pos(leaf, key, lo_key, hi_key));
    ...
    iterator upper_bound(const key_type& key)
    {
        ...
        {
            const inner_node *inner = static_cast<const inner_node*>(n);
            int slot = find_upper(inner, key, predict_pos(inner, key, lo_key, hi_key));
            lo_key = (slot > 0) ? &inner->slotkey[slot - 1] : NULL;
            hi_key = (slot < inner->slotuse) ? &inner->slotkey[slot] : NULL;
        ...
        int slot = find_upper(leaf, key, predict_pos(leaf, key, lo_key, hi_key));
        return iterator(leaf, slot);
    }
...
public:
    ...
    inline mabtree(const btree_self &other)
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_stats( other.m_stats ),
          m_key_less( other.key_comp() ),
          m_allocator( other.get_allocator() )
...
private:
    ...
    struct node* copy_recursive(const node *n)
    {
        if (n->isleafnode())
        {
            ...
            newleaf->midp_err = leaf->midp_err;
        ...
        else
        {
            ...
            newinner->midp_err = inner->midp_err;
...
private:
    ...
    std::pair<iterator, bool> insert_start(const key_type& key, const data_type& value)
    {
        ...
        std::pair<iterator, bool> r = insert_descend(m_root, key, value, &newkey, &newchild, NULL, NULL);
    ...
    */
    std::pair<iterator, bool> insert_descend(node* n,
                                             ...
                                             key_type const* _lo_key, key_type const* _hi_key)
    {
        if (!n->isleafnode())
        {
            ...
            int slot = find_lower(inner, key, predict_pos(inner, key, _lo_key, _hi_key));
            key_type const* lo_key = (slot > 0) ? &inner->slotkey[slot - 1] : NULL;
            key_type const* hi_key = (slot < inner->slotuse) ? &inner->slotkey[slot] : NULL;

            BTREE_PRINT("mabtree::insert_descend into " << inner->childid[slot]);
            ...
            if (newchild)
            {
                BTREE_PRINT("mabtree::insert_descend newchild with key " << newkey << " node " << newchild << " at slot " << slot);

                if (inner->isfull())
                {
                    split_inner_node(inner, splitkey, splitnode, slot);

                    BTREE_PRINT("mabtree::insert_descend done split_inner: putslot: " << slot << " putkey: " << newkey << " upkey: " << *splitkey);
                    ...
                    BTREE_PRINT("mabtree::insert_descend switch: " << slot << " > " << inner->slotuse+1);
                    ...
                    else if (slot >= inner->slotuse+1)
                    {
                        ...
                        BTREE_PRINT("mabtree::insert_descend switching to splitted node " << inner << " slot " << slot);
                    }
                }
        ...
        else // n->isleafnode() == true
        {
            ...
            int slot = find_lower(leaf, key, predict_pos(leaf, key, _lo_key, _hi_key));
    ...
    void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
        ...
        BTREE_PRINT("mabtree::split_leaf_node on " << leaf);
        ...
        compute_midp_err(leaf);
        compute_midp_err(newleaf);
    ...
    void split_inner_node(inner_node* inner, key_type* _newkey, node** _newinner, unsigned int addslot)
    {
        ...
        BTREE_PRINT("mabtree::split_inner: mid " << mid << " addslot " << addslot);
        ...
        BTREE_PRINT("mabtree::split_inner_node on " << inner << " into two nodes " << mid << " and " << inner->slotuse - (mid + 1) << " sized");
        ...
        compute_midp_err(inner);
        compute_midp_err(newinner);
...
public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend)
    {
        ...
        BTREE_PRINT("mabtree::bulk_load, level 0: " << m_stats.itemcount << " items into " << num_leaves << " leaves with up to " << ((iend - ibegin + num_leaves-1) / num_leaves) << " items per leaf.");
        ...
        BTREE_PRINT("mabtree::bulk_load, level 1: " << num_leaves << " leaves in " << num_parents << " inner nodes with up to " << ((num_leaves + num_parents-1) / num_parents) << " leaves per inner node.");
        ...
        for (int level = 2; num_parents != 1; ++level)
        {
            ...
            BTREE_PRINT("mabtree::bulk_load, level " << level << ": " << num_children << " children in " << num_parents << " inner nodes with up to " << ((num_children + num_parents-1) / num_parents) << " children per inner node.");
...
public:
    ...
    {
        BTREE_PRINT("mabtree::erase_one(" << key << ") on btree size " << size());
    ...
    {
        BTREE_PRINT("mabtree::erase_iter(" << iter.currnode << "," << iter.currslot << ") on btree size " << size());
...
private:
                               ...
                               inner_node *parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            int slot = find_lower(leaf, key, predict_pos(leaf, key, NULL, NULL));
        ...
        else // !curr->isleafnode()
        {
            ...
            int slot = find_lower(inner, key, predict_pos(inner, key, NULL, NULL));
                                ...
                                inner_node *parent, unsigned int parentslot)
    {
        ...
        else // !curr->isleafnode()
        {
            ...
            result_t result;
            int slot = find_lower(inner, iter.key(), predict_pos(inner, iter.key(), NULL, NULL));
...
public:
    ...
    bool restore(std::istream &is)
    {
        ...
        {
            BTREE_PRINT("mabtree::restore: file header does not match instantiation signature.");
            return false;
        }
